...
//  Serial.println(fanNumber);
  for(int i = 0; i < 5; i++){
    if(i == fanNumber)digitalWrite(fans[i],HIGH);
    else digitalWrite(fans[i],LOW);
  }
  return;
}
...
void loop() {
  if(Serial.available() > 0){
    ...
    while(Serial.available() > 0){  
      ...
      if(index >= 5)continue; // コマンドの文字数制限
      if(input == '\r' || input == '\n' || input == ' ')continue;
      index++;
    }
    Serial.println(inputCommand);
    
    if(index < 5){Serial.println("too short length");} // out of indexを避ける
    else if(inputCommand[0]=='f'){ // fan10やfan11など
      int fanNum = int(inputCommand[3]-'0');
      int fanVal = int(inputCommand[4]-'0');
      int previousVal = fanStatus[fanNum];
      
      if(previousVal != fanVal){ // 以前に格納されていた値と違った場合のみsumやstatusを変更
        if (fanVal == 1)sumOfHighFan++; 
        else if(fanVal == 0)sumOfHighFan--;
  ...
  for(int i=0; i<5; i++){ // HIGHの個数に合わせた長さでファンを動作
    if(fanStatus[i] == 1){
      onOnlyAFan(i);
      delay(onTimeMS);
    }
